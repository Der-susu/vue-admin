...
export default {
  data(){
    ...
    methods:{
        ...
        async login(){
            const { data: res }= await this.$axios.post("http://localhost:81/user/login",JSON.stringify(this.user),{ headers: {
        ...
        async register(){  
            const { data: res }= await this.$axios.post("http://localhost:81/user/register",JSON.stringify(this.user),{ headers: {
            'Content-Type': 'application/json;charset=UTF-8'
        }})
            if(res.code===20011){
                //注册成功
